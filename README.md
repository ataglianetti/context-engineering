...
Most AI productivity systems give you templates and commands. This gives you the scaffolding that makes them reliable:
...
**`/setup` makes it yours.** The repo is scaffolding: rules, commands, hooks. When you run `/setup`, it interviews you and generates a thinking partner calibrated to your domain, context folders for your work, and memory files that persist across sessions. Ten minutes, and the generic scaffold becomes your system.
...
| `/today` | Morning briefing — meetings, due reminders, meeting-note scaffolding |
...
| `/map-codebase` | Map an existing codebase for agent consumption |

### Agents (7)

Subagent definitions in `.claude/agents/`, spawned by the coding-project commands (`/new-project`, `/map-codebase`) rather than invoked directly. The vault commands all process in context.

| Agent | Role |
|-------|------|
| `planner` | Decomposes a task into a stepwise implementation plan |
| `implementer` | Executes a planned change |
| `verifier` | Checks the change did what the plan said |
| `reviewer` | Reviews for quality and correctness |
| `debugger` | Isolates and diagnoses a failure |
| `researcher` | Gathers and synthesizes external material |
| `mapper` | Builds a structural map of a codebase |

### Hooks (9)

Three kinds of enforcement. Some assert *facts* on every file write. Some keep individual notes (meetings, threads) inside their length budget. And two keep your always-loaded memory from rotting — they make upkeep mechanical instead of relying on you to remember.

Every blocking hook here asserts something that is either true or false, never a house style. Formatting preferences belong in rules, where you can disagree with them; a hook that blocks on format is just a linter with opinions.
...
| `validate-dates.sh` | Ensures memory/work-state dates match today, not an inferred date | Blocks |
| `validate-wikilinks.sh` | Finds wikilinks pointing to non-existent notes | Warns |
| `validate-day-of-week.sh` | In `Calendar/` notes, blocks when a written day name doesn't match the actual day for that date — computing a weekday from a date is arithmetic an LLM fakes fluently | Blocks |

**Note-length budgets** (warn only — friction to prompt a consolidation pass)

| Hook | Behavior | Enforcement |
|------|----------|-------------|
| `validate-meeting-bullets.sh` | Warns when a Meeting note has more than 6 top-level bullets under `## Summary`, **or** when any of those bullets exceeds 180 characters (it names the worst offender). Length matters as much as count: a note can sit at exactly 6 bullets and still be a wall if each bullet is a paragraph | Warns |
...
The frontmatter contract lives in `rules/vault/vault-structure.md`: Meeting, Meeting Series, Thread, Document, Person, Context, Product, Platform, Initiative, Feature, Journal, MOC, Definition, Reference, Post, Framework, and more. `type:` is required on every note and `context:` on context-linked types — both enforced on write by `validate-frontmatter.sh`.

This kit ships the contract, not note templates. Bring your own (Templater or otherwise) and have them emit these fields; the hooks skip `Resources/Templates/` so placeholder scaffolding doesn't trip the type check.

## Using It

After `/setup`, your vault has contexts, people, projects, and rules. Here's how the pieces fit into a daily workflow.

**Morning:** Run `/today` for a briefing — tell it what's on your calendar and it flags which meetings have series notes, surfaces due reminders, and scaffolds the day's meeting notes.
...
**`Resources/`** holds reusable reference material — frameworks you apply across contexts, external articles you've saved, and standard definitions. `Resources/Meta/Claude/` is the system itself: rules, commands, hooks, and agents.
...
This repo contains scaffolding — rules, memory architecture, hooks, commands, agents. It does not contain content — your notes, people, projects, decisions.
