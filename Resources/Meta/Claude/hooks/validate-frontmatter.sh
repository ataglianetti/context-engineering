...
if [[ -z "$TYPE_VALUE" ]]; then
  ...
  jq -n '{
    decision: "block",
    reason: "Missing `type:` property. Add `type:` to frontmatter. Valid types: Context, Product, Platform, Initiative, Feature, Document, Definition, Application, Framework, Reference, Journal, Meeting, Meeting Series, Thread, Person, Post, Song, Object Writing, MOC, Vendor."
  }'
  exit 0
fi
