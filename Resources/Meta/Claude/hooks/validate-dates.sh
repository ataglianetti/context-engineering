...
# Skip OTHER projects' copies (starter kit, /new-project template deploys) — but NOT
# this vault's own files, which legitimately live under Resources/Meta/Claude/rules/core/.
# (Bug fixed 2026-07-06: a blanket rules-path skip here matched the real memory.md and
# work-state.md, silently disabling this hook on the exact files it exists to validate.)
if [[ -n "${CLAUDE_PROJECT_DIR:-}" ]]; then
  case "$FILE_PATH" in
    "$CLAUDE_PROJECT_DIR"/*) ;;
    *)
      echo "$(date '+%H:%M:%S') validate-dates: outside project — skipped ($FILE_PATH)" >> "$LOG"
      exit 0 ;;
  esac
fi
