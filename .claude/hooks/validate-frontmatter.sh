...
# Valid type values
VALID_TYPES="Context|Product|Platform|Initiative|Feature|Document|Definition|Application|Framework|Reference|Journal|Meeting|Meeting Series|Thread|Person|Post|Song|Object Writing|MOC"
...
if [[ -z "$TYPE_VALUE" ]]; then
  jq -n '{
    decision: "block",
    reason: "Missing `type:` property. Add `type:` to frontmatter. Valid types: Context, Product, Platform, Initiative, Feature, Document, Definition, Application, Framework, Reference, Journal, Meeting, Meeting Series, Thread, Person, Post, Song, Object Writing, MOC. Check template at Resources/Templates/[Type].md"
  }'
  exit 0
fi
...
if ! echo "$TYPE_VALUE" | grep -qE "^($VALID_TYPES)$"; then
  jq -n --arg type "$TYPE_VALUE" '{
    decision: "block",
    reason: ("Invalid type: \"" + $type + "\". Valid types: Context, Product, Platform, Initiative, Feature, Document, Definition, Application, Framework, Reference, Journal, Meeting, Meeting Series, Thread, Person, Post, Song, Object Writing, MOC. Check template at Resources/Templates/[Type].md")
  }'
  exit 0
fi

# Context-linked types that require context: property
CONTEXT_TYPES="Product|Platform|Initiative|Feature|Document|Post|Application|Meeting|Thread|MOC"
