...
2. **Copy agent templates** from vault:
   - Read each file from `Resources/Templates/Agents/` (researcher, planner, implementer, verifier, debugger, reviewer)
   - Write to `~/Projects/{name}/.claude/agents/`

3. **Copy map-codebase command** from vault:
   - Read `Resources/Templates/Commands/map-codebase.md`
